...
test('stale projection rows replay durable worker idempotency and disappear after refresh', async () => {
  const state = await fixture();
  ...
  const revisionBeforeExpiry = await state.market.getRevision();
  const first = await worker.runOnce();
  assert.deepEqual(first.skipped, []);
  assert.equal(first.expired.length, 1);
  assert.equal(first.expired[0].orderId, state.due.id);
  assert.equal(await state.market.getRevision(), revisionBeforeExpiry + 1);
  assert.equal((await state.market.listOffers({ status: null })).find((offer) => offer.id === state.expiring.id).remaining, 17);

  const revisionAfterExpiry = await state.market.getRevision();
  const replay = await worker.runOnce();
  assert.deepEqual(replay.skipped, []);
  assert.deepEqual(replay.expired, first.expired);
  assert.equal(await state.market.getRevision(), revisionAfterExpiry, 'idempotent replay must not create another transition');
  assert.equal((await state.market.listOffers({ status: null })).find((offer) => offer.id === state.expiring.id).remaining, 17);
