...
const route = async (req, res, requestId) => {
  ...
  if (req.method === 'POST' && url.pathname === '/v1/assets') {
    return json(res, 201, { data: market.registerAsset(await readBody(req), context(req)) }, { 'x-request-id': requestId });
  }
  ...
  if (req.method === 'POST' && url.pathname === '/v1/offers') {
    return json(res, 201, { data: market.createOffer(await readBody(req), context(req)) }, { 'x-request-id': requestId });
  }
  if (req.method === 'POST' && url.pathname === '/v1/orders') {
    return json(res, 201, { data: market.createOrder(await readBody(req), context(req)) }, { 'x-request-id': requestId });
  }
  ...
  if (req.method === 'POST' && parts[0] === 'v1' && parts[1] === 'orders' && parts.length === 4) {
    ...
    const body = action === 'cancel' ? {} : await readBody(req);
